...
function gear_ratio_table(idx) =
                 //[ ring teeth, planet_teeth, planet_shaft ]
  idx == "4.44"  ? [    31     ,     11      ,      6     ] :
  idx == "4.20"  ? [    32     ,     11      ,      6     ] :
  idx == "4.00"  ? [    30     ,     10      ,      6     ] :
  idx == "3.80"  ? [    28     ,      9      ,      6     ] :
...
gear_ratio_params = gear_ratio_table(gear_ratio);
ring_teeth        = gear_ratio_params[0];
planet_teeth      = gear_ratio_params[1];
planet_shaft_diameter = gear_ratio_params[2];
...
//planetary_gearbox_size = [housing_size, housing_size, (planetary_frame_size[z]-motor_mount_size[z])/2];
...
module planetary_frame()
{
	difference()
	{
		...
		union()
		{
			...
			thickness=planet_carrier_height+vertical_clearance*2+.1;
...
module sun_gear()
{
	difference()
	{
		union()
		{
			...
			thickness=sun_gear_tooth_height;
...
module planet_carrier()
{
	...
	difference()
	{
		...
		{
            a1=i*floor(360/num_planets/planet_angle)*planet_angle+planet_offset;
			rotate([0, 0, a1])
			{
...
module planetary_assembled(exploded=0)
{
	...
	*translate(ring_gear_bottom+[0, 0, vertical_clearance+exploded*24])
	...
	translate(ring_gear_bottom+[0, 0, planet_carrier_gear_center[z]-planet_gear_height/2+exploded*36])
	{
		...
		{
            a1=i*floor(360/num_planets/planet_angle)*planet_angle+planet_offset;
            a2=a1*ring_teeth/planet_teeth+180/planet_teeth*odd(sun_teeth);
			rotate([0, 0, a1])
